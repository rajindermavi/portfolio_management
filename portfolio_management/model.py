...
class AgentComparison():
    ...
    def simulate_agents(self):
        ...
        _loss = agent_loss(self.meta_env.env,
                           ...
                           self.n_stocks)
        self.dpm_vals = self.meta_env.env.portfolio_value_hist

        _loss = agent_loss(self.meta_env.env,
                           self.meta_agent.uniform_agent,
                           self.n_stocks)
        self.uniform_vals = self.meta_env.env.portfolio_value_hist

        _loss = agent_loss(self.meta_env.env,
                           self.meta_agent.mvp_agent,
                           self.n_stocks)
        self.mvp_vals = self.meta_env.env.portfolio_value_hist

        _loss = agent_loss(self.meta_env.env,
                           self.meta_agent.capm_agent,
                           self.n_stocks)
        self.capm_vals = self.meta_env.env.portfolio_value_hist
